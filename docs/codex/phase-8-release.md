...
## Key change
Drop all non-x64 targets to avoid the `internal/goarch` export-data error in GolangCI-Lint.

---

## Deliverables

1. **`goreleaser.yml`** \u2013 **x64-only** builds (`linux/amd64`, `windows/amd64`, `darwin/amd64`), `CGO_ENABLED=0`, archives include README, LICENSE, completions, themes; SHA256 & SBOM.
2. **Cosign signing** \u2013 use repo secrets `COSIGN_PRIVATE_KEY`, `COSIGN_PASSWORD`.
3. **Docker image** \u2013 single-arch amd64 image `ghcr.io/jalsarraf0/ai-chat-cli:{version,latest}` produced via Goreleaser.
4. **Homebrew formula** \u2013 auto\u2011tap `jalsarraf0/homebrew-ai-chat-cli`.
5. **Workflows**
   * **Snapshot** \u2013 extend CI: `make snapshot` (`goreleaser build --snapshot`).
   * **Release** \u2013 job in `ci.yml`, trigger on `v*.*.*` tag, runs `goreleaser release --clean --sign` on Linux self\u2011hosted.
6. **Makefile targets**
   ```make
   snapshot:
...
8. **Tests to keep \u2265\u202f92\u202f% coverage** (current suite ~95%)
...
Arm64 binaries may be added once the GolangCI-Lint issue is resolved.
