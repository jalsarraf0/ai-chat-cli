...
func New() Client {
	...
	return Client{key: key, base: base, http: &http.Client{Timeout: timeout}}
}
...
func (c Client) Completion(ctx context.Context, req llm.Request) (llm.Stream, error) {
	...
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost, c.base+"/v1/chat/completions", &body)
	if err != nil {
		return nil, err
	}
